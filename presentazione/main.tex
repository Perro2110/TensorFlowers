	...
	\begin{frame}{\centering Struttura della Rete, Blocco Convoluzionale}
		\begin{block}{Blocco Convoluzionale $\times 4$}
			...
			\begin{itemize}
				...
				\item MaxPooling (per riduzione dimensionalita dell'immagine)
			\end{itemize}
		\end{block}
...
\begin{frame}{\centering Risultati: Confronto Modelli}
    ...
    \begin{adjustbox}{width=\textwidth}
        ...
        \begin{tabular}{lcccc}
            \toprule
            \textbf{Modello} & \textbf{Avg Val Acc (\%)} & \textbf{Avg Val Loss}
            & \textbf{Test Acc (\%)} & \textbf{Epoche}\\
            \midrule
            \textbf{CNN Custom}        & 79.1\% & 0.53 & 79.4\% & $\sim$40 \\
            \textbf{CNN Custom + Augmentation}        & 81.7\% & 0.42 & 82.9\% & $\sim$50 \\
            Bagging (5 checkpoint)   & -- & -- & 86.1\% & -- \\
            ResNet-50 Fine Tuning & 90\% & 0.25 & 90.6\% & 20 \\
            \bottomrule
        \end{tabular}
    \end{adjustbox}
...
\begin{frame}{\centering Conclusioni}
    ...
    \onslide<2->{
        \begin{block}{CNN Custom}
            ...
            bagging, raggiunge circa \alert{86\% di accuracy},
    ...
    \onslide<3->{
        \begin{block}{Fine Tuning ResNet-50}
            Il transfer learning su ResNet-50 raggiunge \alert{90\%}
